...
in
{
  ...
  environment.etc."vim/vimrc" = {
    text = ''
      ...
      " 允许用户覆盖系统配置
      " 若通过 `system.activationScripts.createVimrc` 创建链接则会导致无限递归
  ...
  system.activationScripts.createVimrc = ''
    if [ -d "/home/${username}" ]; then
      ln -sf /etc/vim/vimrc /home/${username}/.vimrc
      # chown ${username}:users /home/${username}/.vimrc  # rofs can not change owner
    fi
  '';
